  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (isBillingHealth && request.method === "GET") {
      ...
      try {
        ...
        const response = await env.BILLING.fetch(new Request(probeUrl.toString(), { method: "GET", headers }));
        ...
        console.error("billing service binding health check failed", error);
        return json({ error: { code: "BILLING_BINDING_UNAVAILABLE", message: "Billing service is unavailable", request_id: id } }, 503, { "x-request-id": id });
      }
    }
    ...
    if (!url.pathname.startsWith("/v1/") && !isAuth && !isCustomerProvisioning && !isRazorpayWebhook && !isBillingClaim && !isBillingHealth && !isCheckout) return json({ error: { code: "NOT_FOUND", message: "Route not found", request_id: id } }, 404, { "x-request-id": id });
...
// Release 1.9.2: add /billing/health to exercise the BILLING service binding directly.
