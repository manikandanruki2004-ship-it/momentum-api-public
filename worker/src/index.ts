...
function json(body: unknown, status = 200, headers: HeadersInit = {}) {
  return new Response(JSON.stringify(body), { status, headers: { "content-type": "application/json; charset=utf-8", ...corsHeaders, ...headers } });
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const isAuth = url.pathname === "/auth/google";
    ...
    const isAllowedPost = (isAuth || isCustomerProvisioning || isRazorpayWebhook || isBillingClaim) && request.method === "POST";
    if (request.method !== "GET" && !isAllowedPost) {
      return json({ error: { code: "METHOD_NOT_ALLOWED", message: "Method not allowed", request_id: id } }, 405, { "allow": "GET,POST,OPTIONS", "x-request-id": id });
    }
    ...
    if (url.pathname === "/version") return json({ name: "Momentum API", version: "1.5.0", engine: "1.3.1", billing: "1.1.0", auth: "1.0.0" }, 200, { "x-request-id": id });

    if (!url.pathname.startsWith("/v1/") && !isAuth && !isCustomerProvisioning && !isRazorpayWebhook && !isBillingClaim) {
...
// Release 1.5.0: Google sign-in is public; Free plan exposes 10 results/request; billing claim remains available.
